...
<div class={cn(baseClass, 'hidden sm:flex')}>
	<div class="flex items-center gap-2">
		<a href="/">
			<img class="max-h-8 max-w-8" alt={'SKO logo'} src={'/logo.png'} />
		</a>
	...
	<div class="flex items-center gap-2">
		...
		<form method="post" action="/api/auth/logout">
			<Button size="sm" type="submit" variant="outline" on:click={handleSignOut}>Sign out</Button>
		</form>
	</div>
</div>
		...
		<Sheet.Root bind:open>
			...
			<Sheet.Content side={'left'}>
				<NavbarLink href={'/'} bind:open class="flex items-center">
					<img class="mr-2 h-6 w-6 object-contain" alt={'SKO logo'} src={'/logo.png'} />
					<span class="font-bold">{applicationName}</span>
				</NavbarLink>
				<div class="my-4 h-[calc(100vh-6rem)] overflow-auto pb-6 pl-6">
							...
							<div class="flex flex-col space-y-2">
								...
								{#if navItem?.items?.length}
									{#each navItem.items as item}
										{#if item.href}
											<NavbarLink href={item.href} bind:open class="text-muted-foreground">
												{item.title}
											</NavbarLink>
										{/if}
									{/each}
