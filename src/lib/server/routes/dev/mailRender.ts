...
export const mailDevRouter = router({
  mailRender: publicProcedure.input(z.object({ template: z.string(), type: z.enum(["svelte", "text"]) })).query(({ input }) => {
    const content = getMailContent(input.type, input.template, {});
    if (!content) {
      throw new TRPCError({ code: "NOT_FOUND", message: `No mail template for ${input.template} of type ${input.type}` })
    }
    return content;
  }),
})
