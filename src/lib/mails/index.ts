...
type TemplateFileLoad = { default: new (...args: unknown[]) => SvelteComponent<Record<string, unknown>> };
...
export const sendMail = async (recepient: string, templateName: string, props?: Record<string, unknown>) => {
	if (dev || building) {
		...
		const urlParams = new URLSearchParams({ template: templateName, props: JSON.stringify(props ?? {}) });
...
const getSvelteMailContent = (templateName: string, props: Record<string, unknown>) => {
	const htmlContent = (svelteEntries[`./templates/${templateName}.html.svelte`] as TemplateFileLoad)?.default;
	if (!htmlContent) {
		return;
	}
...
const getTextMailContent = (templateName: string, props?: Record<string, unknown>) => {
	const textContent = (textEntries[`./templates/${templateName}.text.svelte`] as TemplateFileLoad)?.default;
	if (!textContent) {
		return;
	}
